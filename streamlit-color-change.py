...
#do I need to call integer mask here?/new
def AvgColor(image):  
    colList=[]
    width = image.shape[0]
...
if uploaded_image is not None:
    ...
    # Add a text input field for the user to enter a color name
    color_name = st.text_input("Enter a color name:")
    
    #Remove spaces from color_name
    color_name =  color_name.replace(" ", "")

    # Retrieve RGB value based on the entered color name
    rgb_value = get_rgb_from_color_name(color_name) 
    ...
    #get color changed image
    image = colorChange3(rgb_value, image)
